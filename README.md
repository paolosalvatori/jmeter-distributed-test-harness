...
Apache JMeter distributed testing leverages multiple systems to perform load testing against a target system, typically a web site or REST API. Distributed testing can be used to sping up a large amount of concurrent virtual users and generate traffic aginst websites and server applications. The ARM template can be used to deploy an [Apache JMeter Distributed Test Harness](https://jmeter.apache.org/usermanual/jmeter_distributed_testing_step_by_step.html) composed of Azure virtual machines located in different Azure regions. There are other projects available on GitHub that allow to build a distributed test harness on Azure. By the way, these projects use, respectively, Azure Kubernetes Servicer (AKS) and Azure Container Instances (ACI) to build a JMeter distributed test harness in a single virtual network and region:
